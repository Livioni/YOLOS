...
class DynamicDetector(Detector):
    def __init__(self, num_classes, pre_trained=None, det_token_num=100, backbone_name='tiny', init_pe_size=[800, 1344], mid_pe_size=None, use_checkpoint=False,\
                 pruning_loc = [3,6,9],keep_rate=.9):
        super().__init__(num_classes, pre_trained, det_token_num, backbone_name, init_pe_size, mid_pe_size, use_checkpoint)
        if backbone_name == 'tiny':
            self.backbone, hidden_dim = dynamic_tiny(pretrained=pre_trained, pruning_loc=pruning_loc,\
                                                     keep_rate=keep_rate)
        elif backbone_name == 'small':
            self.backbone, hidden_dim = small(pretrained=pre_trained)
        elif backbone_name == 'base':
...
class SetDynamicCriterion(nn.Module):
    """ This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    """
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, keep_ratio = [0.9,0.9**2,0.9**3]):
        """ Create the criterion.
        Parameters:
            num_classes: number of object categories, omitting the special no-object category
            matcher: module able to compute a matching between targets and proposals
    ...
    def loss_ratio(self, outputs, targets, indices, num_boxes):
        out_pred_score = outputs['out_pred_prob']
        pred_loss = 0.0
